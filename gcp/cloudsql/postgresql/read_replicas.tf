...
resource "google_sql_database_instance" "replicas" {
  ...
  region               = join("-", slice(split("-", lookup(each.value, "zone", var.location)), 0, 2))
  ...
  settings {
    ...
    user_labels     = merge({"terraform": true},lookup(each.value, "user_labels", var.user_labels))
    ...
    location_preference {
      zone = lookup(each.value, "zone", var.location)
    }
